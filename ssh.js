...
var cps, exec, nexpect, parseIperfRow, parseStatus, parseUFTP, _;
...
exports.cancel = function() {
  ...
  for (_i = 0, _len = cps.length; _i < _len; _i++) {
    cp = cps[_i];
    _results.push(cp.kill("SIGHUP"));
  }
  return _results;
};

exec = function(cmd, cb, stream) {
  var cp;
  if (stream == null) {
    stream = "stdout";
  }
...
exports.config = function(harq, arq, nc, cb) {
  return exec("ncdemo/bs.sh " + harq + " " + arq, function(err, result) {
    return exec("ncdemo/setup.sh " + nc, function(err, result) {
      return typeof cb === "function" ? cb(err, result) : void 0;
    });
  });
};
